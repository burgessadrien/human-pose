...
def findTorso(mask):
    torsoAverage = 0
    torso = np.zeros((int(mask.shape[0] / 2), (int(mask.shape[1] / 2)), 3), np.uint8)
    torsoWidth = torso.shape[0]
    torsoHeight = torso.shape[1]
    c,d = 0,0
    for i in range(torso.shape[0], mask.shape[0],1):
        for j in range(torso.shape[1], mask.shape[1],1):
            tmp = mask[(i - torsoWidth):i, (j - torsoHeight):j]
            average = np.average(tmp[0:])
            if average > torsoAverage:
                torsoAverage = average
                c,d = i,j

    torso = mask[(c - torsoWidth):c,(d - torsoHeight):d]
    return [torso,[c,d]]
...
def readVideo():
    ...
    while (k%256 != 27):
        ...
        threshHuman = np.zeros((width, height, 3), np.uint8)
        human = np.zeros((width, height, 3), np.uint8)
        torso = np.zeros((width, height, 3), np.uint8)
        (rects, weights) = hog.detectMultiScale(frame, winStride=(5, 5),padding=(30, 30), scale=1.05)
        rects = np.array([[x, y, x + w, y + h] for (x, y, w, h) in rects])
        pick = non_max_suppression(rects, probs=None, overlapThresh=0.65)
        tmp = np.zeros((int(human.shape[0] / 2), (int(human.shape[1] / 2)), 3), np.uint8)
        if len(pick) > 0:
            print("doing stuff")
            (xA, yA, xB, yB) = pick[0]
        ...
        cv2.imshow('tmp', np.array(torso, np.uint8))
